...
the streak/cooldown state - LANDED as private fields on a
module-singleton instance), and the orchestrator `ChannelCoordinator`
(wraps the entry file) - with the
...
| `src/channel-coordinator/liveness.ts` | 288 | the liveness *probe* functions stay free - they are pure utility functions consumed by both `ChannelCoordinator` and 2 external web/ files (G.3 `LivenessTracker` class lands in `channel-coordinator.ts`, NOT here; the streak state is now 4 private fields on a module-singleton `LivenessTracker` instance - see `03-class-boundaries.md §G3`) | G.3 [LANDED] |
...
2. **The 4 mutable lets at the streak-state declaration block in `channel-coordinator.ts` (`state`, `downStreak`, `stopping`, `nativeConfirmedUpUntil`) must move together into one class instance - RESOLVED via G.3.** Per `01 §1.2`, `runLoop` reads AND writes all 4 within its `while (!stopping)` body (the L322-339-372-393 reads/writes are interleaved across all four). Splitting them across 4 separate classes introduces ordering hazards; collapsing them into 4 private fields of `LivenessTracker` is the only safe move. G.3 LANDED this collapse (commit (this commit)); the 15 read/write sites route through the class methods and the free `inNative409Cooldown` helper survives as a 1-line shim. After class extraction, a race between `stop()` (the SIGTERM latch) and `runLoop()` is structurally impossible (single-threaded JS, single process) but the `setTimeout(…, 3000)` in the shutdown drain handler introduces a 3-second window where `stopping = true` and `closeIngestDb()` is deferred - G.3 preserved this 3-second drain. Detail in `06-risks-and-mitigations.md` GR2.
...
G.2  IngestWorker class extraction         (introduce alongside; COORDINATOR_AGENT_ID re-exported)
  |
G.3  LivenessTracker class extraction      LANDED (this commit)
  |
...
- **G.3 LANDED** (this commit) - the 4 mutable lets at the streak-state declaration block in `channel-coordinator.ts` are now 4 private fields on a module-singleton `LivenessTracker` instance. The G.3 class is the smallest of the 4 classes (4 fields + 9 thin getter/setter methods + the `inNative409Cooldown` helper method). The actual liveness *probe* functions in `liveness.ts` stay free per the G.3 design (per `03-class-boundaries.md §G3 "Free functions that REMAIN"`).
...
- `src/channel-coordinator.ts` - `class LivenessTracker` (G.3 LANDED - was the 4 module-level lets at the streak-state declaration block; now 4 private fields on a module-singleton instance, see `03-class-boundaries.md §G3`)
