...
current form. The 4 module lets at the streak-state declaration block
in `channel-coordinator.ts` (`state`, `downStreak`, `stopping`,
`nativeConfirmedUpUntil`) are **LANDED** as private fields on
`LivenessTracker` per `03-class-boundaries.md §G3` (this commit). **`ChannelCoordinator` is the only G
...
- **Interaction with the 4 `channel-coordinator.ts` state lets
  [G.3 LANDED].** `liveness.ts` holds **none** of them. The coupling
  is one-directional and purely by value: `runLoop` calls
  `probeNativeChannelDown(SESSION, PROVIDER)` at the runLoop state-
  machine flow and folds the `boolean` result into `downStreak`
  (now `liveness.incrementDownStreak()` / `liveness.resetDownStreak()`)
  and the `state` transitions (now `liveness.setState(...)`).
  `nativeConfirmedUpUntil` (now `liveness.nativeConfirmedUpUntil`)
  never reaches `liveness.ts` at all - the 409 cooldown is applied on
  the coordinator side at the idle branch via
  `!liveness.inNative409Cooldown(Date.now())`. **This is why
  `LivenessTracker` is sourced from the streak-state declaration block
  in `channel-coordinator.ts` and not from `liveness.ts`** (per
  `03 §G3`, LANDED this commit): the two files share no mutable
  state, only a boolean return value.
...
`type State = 'idle' | 'backfilling'` (declared at the streak-state
block in `channel-coordinator.ts`) is a 2-variant union used in exactly
one file, and the tracker's other three fields (`downStreak: number`,
`stopping: boolean`, `nativeConfirmedUpUntil: number`) are not
parameterizable at all. A `TState` parameter would generalize one of
four fields for one consumer. `review-correctness.md` §Out-of-scope
cross-checks separately confirms that `review-completeness.md` CE-9's
`RemoteStatusCache<T>` reuse opportunity does **not** apply here:
`web/remote-status-cache.ts` is imported by zero G files, and a
TTL-keyed status cache is not the shape of a 4-field state machine.
**Rejected per OE-6; CE-9 not applicable.** **Note:** `LivenessTracker`
landed (G.3 LANDED, this commit) without a `TState` parameter - the
class surface in `03 §G3` is non-generic, matching this rejection.
