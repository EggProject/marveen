...
| **C → E** | `PortLockAcquirer` / `PidfileLockAcquirer` (`e-process-lock/00-summary.md`) | `index.ts:350` calls `new PortLockAcquirer(procCtx).acquire(WEB_PORT, ...)` before `startWebServer`; the lock outlives the server. `DashboardServer.stop()` is called inside `webServer.close` callback (per `index.ts:386-422`), which is AFTER `releaseLock()` (now at `index.ts:364-371`, a thin wrapper around `pidfileLockAcquirer.release(PID_FILE, process.pid)`). C is downstream of E. | No. |
