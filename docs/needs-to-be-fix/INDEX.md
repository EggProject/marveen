...
| `heartbeat-brief-rundiceaysweep-not-applicable` | `src/heartbeat.ts` (no symbol) | task brief mentions `runDecaySweep` integration but the integration does not exist | `src/__tests__/heartbeat-cov.test.ts` | Resolved: 2026-08-25 (this commit) (runDecaySweep opportunistically invoked at the start of every in-window executeHeartbeat tick; try/catch swallows failures; 2 new tests in heartbeat-cov.test.ts) |
