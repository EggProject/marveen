...
| `heartbeat-brief-rundiceaysweep-not-applicable` | `src/heartbeat.ts` (no symbol) | task brief mentions `runDecaySweep` integration but the integration does not exist | `src/__tests__/heartbeat-cov.test.ts` | Reopened 2026-08-25 (Deferred to next cycle) -- see MD `## Scope note` |
...
| `message-router-cache-fallback-unreachable` | message-router.ts: cached session-lookup `??` fallback arms are unreachable | Partial: 2026-08-25 900cdb6 (all 3 `??` arms dropped at 481-483; 1 `isMainAgent` ternary arm at line 483 deferred; 97.82% -> 99.24%) -- see message-router-cache-fallback-unreachable.md |
| `message-router-dead-defensive-branches` | message-router.ts: three dead defensive branches block 100% branch coverage | Partial: 2026-08-19 ba6faf8 deleted 2 dead arms; 2026-08-25 900cdb6 dropped all 3 `??` arms at 481-483 (1 isMainAgent ternary deferred); 97.82% -> 99.24% |
| `message-router-unreachable-defensive-branches` | message-router.ts: five unreachable defensive branches block 100% branch coverage | Partial: 2026-08-19 ba6faf8 deleted 2 dead arms; 2026-08-25 900cdb6 dropped all 3 `??` arms at 481-483 (1 isMainAgent ternary deferred); 97.82% -> 99.24% |
...
| `schedule-mcp-precheck-subtree-cycle-defensive` | schedule-mcp-precheck.ts: collectSubtreeCmdlines cycle guard is only reachable through malformed ps output | Reopened 2026-08-25 (Deferred to next cycle) -- see MD `## Scope note` |
