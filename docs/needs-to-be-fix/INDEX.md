...
| `channel-poller-reap-botpid-killed-without-identity-check` | `src/web/channel-poller-reap.ts:76-88,202-230` | `reapChannelOrphans` SIGTERM+SIGKILLs the `bot.pid` pid with no identity check (only `> 1`); nothing ever deletes `bot.pid`, so a stale file plus pid reuse kills an unrelated process while logging "orphans killed" | `src/__tests__/channel-poller-reap.test.ts` | Resolved: 2026-08-20 PLACEHOLDER |
