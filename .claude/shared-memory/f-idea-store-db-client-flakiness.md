...
PRE-EXISTING -- a Honcho `code-review-skipped-findings-dbts-lock-filename` entry dokumentálja a `src/db.ts:2519-2521` dirname relative dbPath shared lock dir findinget (`join(dirname(dbPath), '.task_runs_migrate.lock')`).

## Refaktor scope
A refaktor a `src/db.ts:1672-1799` klaszterre korlátozódott (class IdeaStore + module-singleton + 10 thin shim wrapper). A `DbClient` kódját NEM érintette. A `routes/ideas.ts` UNCHANGED. A db.ts 155 exportált free fn-jéből 10 lett shim, a másik 145 UNCHANGED.

## Javítás
Külön ciklusban, a Honcho `code-review-skipped-findings-dbts-lock-filename` entry-vel összhangban. A `src/db.ts:2519-2521` dirname relative dbPath shared lock dir finding (3 db-client test fixture block) release-checklist-prevention-re. A "release előtt javítjuk" NEM opció, mert "el lesz felejtve" -- 2026-09-06 f84a8666 precedens.
