...
export class StoreWatcher {
  ...
  private readonly onFsEvent = (eventType: string, filename: string | null): void => {
    ...
    if (this.recentEvents.size > 200) {
      ...
      // Hard-cap fallback: when the per-entry DEDUP_MS prune deletes 0 entries
      // (every existing entry is fresh), a sustained burst can grow past 500
      // with no prune happening. The fallback clears the Map when size > 500
      // AND prune === 0, bounding memory under pathological write bursts.
