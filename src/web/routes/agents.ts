...
export async function tryHandleAgents(ctx: RouteContext, webDir: string): Promise<boolean> {
  ...
  //
  // NOTE: the Claude options are also static <option> elements in
  // web/index.html (wizard + edit panel), so this array is an API-only listing
  // today; keep it in sync with those options so a client that does read it (or
  // a future refactor that drops the static markup) does not silently offer a
  // stale set.
