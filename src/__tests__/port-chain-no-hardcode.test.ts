...
/** A throwaway install tree whose canonical config selects a non-default port. */
function makeInstall(port: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'portchain-'))
  mkdirSync(join(dir, 'scripts'), { recursive: true })
  mkdirSync(join(dir, 'scripts', 'lib'), { recursive: true })
  mkdirSync(join(dir, 'store'), { recursive: true })
  writeFileSync(join(dir, 'store', 'config-overrides.json'), `{"WEB_PORT":${port}}\n`)
  cpSync(join(ROOT, 'scripts/lib/runtime-config.sh'), join(dir, 'scripts/lib/runtime-config.sh'))
  return dir
}
...
function makeInstallWithCanonicalStore(port: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'portchain-canon-'))
  mkdirSync(join(dir, 'scripts'), { recursive: true })
  mkdirSync(join(dir, 'store'), { recursive: true })
  writeFileSync(join(dir, 'store', 'config-overrides.json'), `{"WEB_PORT":${port}}\n`)
  return dir
...
describe('PORTCHAIN1: the port chain follows WEB_PORT on a NON-default port', () => {
  ...
  // The shell bridge reads from store/config-overrides.json (canonical store),
  // not from .env. Re-stage the throwaway install accordingly. We also stage the
  // dist/ modules the runtime-config.mjs bridge requires, so the call survives
  // outside the repo checkout.
  function makeInstallCanonical(port: string): string {
    const dir = mkdtempSync(join(tmpdir(), 'portchain-canon-'))
    mkdirSync(join(dir, 'scripts'), { recursive: true })
    mkdirSync(join(dir, 'scripts', 'lib'), { recursive: true })
    ...
    writeFileSync(join(dir, 'store', 'config-overrides.json'), `{"WEB_PORT":${port}}\n`)
    cpSync(join(ROOT, 'scripts/lib/runtime-config.sh'), join(dir, 'scripts/lib/runtime-config.sh'))
    cpSync(join(ROOT, 'scripts/runtime-config.mjs'), join(dir, 'scripts/runtime-config.mjs'))
    for (const f of ['config-resolution', 'config-registry', 'paths']) {
      cpSync(join(ROOT, 'dist', `${f}.js`), join(dir, 'dist', `${f}.js`))
    }
    return dir
  }
  ...
  ])('%s resolves WEB_PORT from the canonical store, not 3420', (script) => {
    const canon = makeInstallCanonical(PORT)
    try {
      cpSync(join(ROOT, script), join(canon, script))
      const body = readFileSync(join(canon, script), 'utf-8')
      const idiom = body.split('\n').filter((l) => l.startsWith('WEB_PORT=')).join('\n')
      const out = execFileSync('bash', ['-c',
        `cd '${canon}' && . scripts/lib/runtime-config.sh && runtime_config_init '${canon}' >/dev/null 2>&1 && ${idiom}\necho "$WEB_PORT"`], { encoding: 'utf-8' })
      expect(out.trim()).toBe(PORT)
    } finally {
  ...
  it('the resolution falls back to 3420 when neither env nor the canonical store selects a port', () => {
    ...
    try {
      cpSync(join(ROOT, 'scripts/doctor.sh'), join(bare, 'scripts/doctor.sh'))
      cpSync(join(ROOT, 'scripts/lib/runtime-config.sh'), join(bare, 'scripts/lib/runtime-config.sh'))
      cpSync(join(ROOT, 'scripts/runtime-config.mjs'), join(bare, 'scripts/runtime-config.mjs'))
      for (const f of ['config-resolution', 'config-registry', 'paths']) {
        cpSync(join(ROOT, 'dist', `${f}.js`), join(bare, 'dist', `${f}.js`))
      }
      const idiom = readFileSync(join(bare, 'scripts/doctor.sh'), 'utf-8')
        .split('\n').filter((l) => l.startsWith('WEB_PORT=')).join('\n')
      const out = execFileSync('bash', ['-c',
        `cd '${bare}' && . scripts/lib/runtime-config.sh && runtime_config_init '${bare}' >/dev/null 2>&1 && ${idiom}\necho "$WEB_PORT"`], { encoding: 'utf-8' })
      expect(out.trim()).toBe('3420')
    } finally {
  ...
  it('leaves the documented fallbacks alone', () => {
    const stays: Array<[string, RegExp]> = [
      ['src/config.ts', /WEB_PORT = Number\(cfg\('WEB_PORT'\)\)/],
