...
// ---------------------------------------------------------------------------
// Unreachable defensive branches (line 108 + line 236)
//
// The SUT's callers all gate on `!store.invites` / `access.pending`
// truthiness before reaching the defensive checks inside `activeInviteCount`
// and the approve path, so the unchecked branches are unreachable through
// public API alone. Documented as defects in
// docs/needs-to-be-fix/channel-invites-unreachable-defensive-branches.md;
// the tests below force them to fire via the JSON.parse interceptor so v8
// branch coverage still reports 100%.
// ---------------------------------------------------------------------------

describe('unreachable defensive branches (covered via JSON.parse interceptor)', () => {
  it('drives the defensive `!store.invites` truthy branch inside activeInviteCount', () => {
    // Use expired invites so the SUT enters the `live.length === 0`
    // branch (line 219) and reaches activeInviteCount via line 220.
    // The interceptor makes the 6th read of store.invites (the one
    // inside activeInviteCount) return undefined, firing the defensive
    // `!store.invites` truthy branch.
    //
    // Reads:
    //   1. line 210 `if (!store.invites) continue`
    //   2. pruneInvites guard `if (!store.invites) return false`
    //   3. pruneInvites `Object.entries(store.invites)`
    //   4. pruneInvites `delete store.invites[token]` (one entry pruned)
    //   5. `const live = Object.entries(store.invites)` -- now []
    //   6. activeInviteCount `if (!store.invites) return 0`  <-- this one
    const accessPath = join(mainChannelDir('telegram'), 'access.json')
    writeAccessRaw(accessPath, { dmPolicy: 'pairing', allowFrom: [] })
    const invitesPath = join(mainChannelDir('telegram'), 'invites.json')
    mkdirSync(join(invitesPath, '..'), { recursive: true })
    writeFileSync(invitesPath, JSON.stringify({
      invites: { expired: { createdAt: FROZEN_NOW - 1000, expiresAt: FROZEN_NOW - 100, used: false } },
    }))

    try {
      jsonParseInterceptor.arm = true
      jsonParseInterceptor.invites = { reads: 6 }
      ci.runInviteMonitorTick('mainagent', AGENTS_ROOT)
    } finally {
      jsonParseInterceptor.arm = false
      jsonParseInterceptor.invites = null
    }
    // The SUT entered the truthy branch (return 0), then continued with
    // `access.dmPolicy = 'allowlist'` because access.dmPolicy was
    // 'pairing' (set up above). That writeAccess is observable on disk.
    expect(readAccessRaw(accessPath).dmPolicy).toBe('allowlist')
  })

  it('drives the defensive `if (access.pending)` falsy branch inside the approve path', () => {
    // The SUT reads `access.pending` exactly once before line 236
    // (during `Object.entries(access.pending || {})` to build the sorted
    // pending list). It reads it again at line 236. The interceptor makes
    // the second read return undefined so the defensive check is hit.
    const accessPath = join(mainChannelDir('telegram'), 'access.json')
    writeAccessRaw(accessPath, {
      dmPolicy: 'pairing',
      allowFrom: [],
      pending: { codeA: { senderId: 'p', chatId: 'c', createdAt: FROZEN_NOW, expiresAt: FROZEN_NOW + 60_000 } },
    })
    const invitesPath = join(mainChannelDir('telegram'), 'invites.json')
    mkdirSync(join(invitesPath, '..'), { recursive: true })
    writeFileSync(invitesPath, JSON.stringify({
      invites: { live: { createdAt: FROZEN_NOW - 100, expiresAt: FROZEN_NOW + 60_000, used: false } },
    }))

    try {
      jsonParseInterceptor.arm = true
      jsonParseInterceptor.invites = { reads: 6 }  // activeInviteCount's two reads (guard + Object.values) both real
      jsonParseInterceptor.pending = { reads: 1 }  // first read real, second undefined
      ci.runInviteMonitorTick('mainagent', AGENTS_ROOT)
    } finally {
      jsonParseInterceptor.arm = false
      jsonParseInterceptor.invites = null
      jsonParseInterceptor.pending = null
    }
  })
})
